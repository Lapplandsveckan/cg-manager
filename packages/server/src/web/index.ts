...
// `/` → `/play` redirect would silently vanish. A literal require() is picked
// up by pkg's static analysis (bundling next.config.js into the snapshot) and
// the resolved object is handed to next({ conf }), which bypasses the disk
// lookup entirely.
