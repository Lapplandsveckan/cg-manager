...
function validateSource(src: any): boolean {
    ...
    if (src.type === 'decklink') return typeof src.device === 'number' && typeof src.format === 'string';
    if (src.type === 'video')    return typeof src.video === 'string' && src.video.length > 0;
    if (src.type === 'channel')  return typeof src.channel === 'number';
    if (src.type === 'color')    return typeof src.color === 'string' && src.color.length > 0;
    return false;
}
...
function validateDestination(dest: any): boolean {
    ...
    if (typeof dest.effectLayer !== 'string' || dest.effectLayer.length === 0) return false;
    if (dest.index !== undefined && typeof dest.index !== 'number') return false;
    return true;
}
